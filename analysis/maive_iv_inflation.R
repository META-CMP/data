...
maive_df_first_inflation<-data.frame(object)
maive_df_second_inflation<-data.frame(object2)
maive_list<-list()
maive_first <- list()
...
for (x in periods) {
  ...
  maive_df_first_inflation[[new_col_name]] <- value
  maive_df_second_inflation[[new_col_name]] <- value2
