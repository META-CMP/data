...
## FAT-PET unweighted
## PEESE
## WAAP (2.8, ap observation selection within period, mixing models across periods)
## WAAP (2.8, ap observation selection within period, mixing models across periods) unweighted
## AK
# Sampling: 10 samples per period (mixing models across horizons)
...
perform_meta_analysis <- function(data, wins, se_opt = "upper") {
  list(
...
analyze_subsample_period <- function(subsample_id) {
...
results <- lapply(1:10, analyze_subsample_period)
...
# For interest rate ----
out_var <- "rate"
# Set new winsorization levels for estimation of rate (higher levels fail due to singularity issues)
