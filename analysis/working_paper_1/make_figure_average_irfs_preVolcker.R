...
# Robustness check period pre-Volcker: Use only Pre-Volcker period for US ----
d_no_qc <- d_no_qc %>%
  filter(us == 1) %>%
  filter(start_date >= "1959-01-01",end_date   <= "1979-06-01")
...
# Save data
#write_csv(avg_irf_output_median$data, file.path(save_path, "avg_irf_output_median_preVolcker.csv"))
...
# Export PDF
out_pdf <- file.path(save_path, "avg_irf_output_median_preVolcker.pdf")
ggsave(
  filename = out_pdf,
  plot     = p_out,
  width    = 360/72,
...
# Save data
#write_csv(avg_irf_pricelevel_median$data, file.path(save_path, "avg_irf_pricelevel_median_preVolcker.csv"))
...
# Export PDF
out_pdf <- file.path(save_path, "avg_irf_pricelevel_median_preVolcker.pdf")
ggsave(
  filename = out_pdf,
  plot     = p_out,
  width    = 360/72,
...
# Save data
#write_csv(avg_irf_rate_median$data, file.path(save_path, "avg_irf_rate_median_preVolcker.csv"))
...
# Export PDF
out_pdf <- file.path(save_path, "avg_irf_rate_median_preVolcker.pdf")
ggsave(
  filename = out_pdf,
  plot     = p_out,
  width    = 360/72,
