...
# Robustness check 15: Use only Local Projections ----
d_no_qc <- d_no_qc %>% 
  filter(group_est_broad %in% c("lp_ardl"))

# Sub-folder for figures ----
subfolder <- "lp_ardl" # Robustness check 15
# This version:
# Methods:
## FAT-PET unweighted
## FAT-PET
...
chosen_periods <- seq(0, 48, by = 3)

# For output ----
out_var <- "output"

# Function to perform multiple meta-analyses for a given wins level for output
perform_meta_analysis <- function(data, wins, se_opt = "lower", waap_horizon = 12) {
  list(
...
write_csv(figure_irf_range_correction_output$data, here::here(paste0("analysis/working_paper_1/figures/irf_range_correction/", subfolder, "/figure_irf_range_correction_output_robust_15.csv")))

# Export to PDF with ggplot (It does not work orca) ----

df_plot <- figure_irf_range_correction_output$data

p_out <- ggplot(df_plot, aes(x = period.month)) +
  
  # Blue confidence bands (average IRF)
  geom_ribbon(aes(ymin = avg_CI.lower_95, ymax = avg_CI.upper_95),
              fill = "deepskyblue3", alpha = 0.12) +
  geom_ribbon(aes(ymin = avg_CI.lower_68, ymax = avg_CI.upper_68),
              fill = "deepskyblue3", alpha = 0.28) +
  
  # Green publication-bias corrected ranges
  geom_ribbon(aes(ymin = correction.min_estimate_95, ymax = correction.max_estimate_95),
              fill = "darkgreen", alpha = 0.20) +
  geom_ribbon(aes(ymin = correction.min_estimate_68, ymax = correction.max_estimate_68),
...
# --- Export to PDF ---
out_pdf <- here::here(
  paste0("analysis/working_paper_1/figures/irf_range_correction/", subfolder,
         "/figure_irf_range_correction_output_robust_15.pdf")
)
...
perform_meta_analysis <- function(data, wins, se_opt = "lower", waap_horizon = 6) {
  list(
    fatpet_uw = meta_analysis(
      data = data,
      outvar = out_var,
...
figure_irf_range_correction_pricelevel$data <- figure_irf_range_correction_pricelevel$data %>%
  ...
  left_join(mean_median_per_period, by = "period.month")
# Save figure data as csv
write_csv(figure_irf_range_correction_pricelevel$data, here::here(paste0("analysis/working_paper_1/figures/irf_range_correction/", subfolder, "/figure_irf_range_correction_pricelevel_robust_15.csv")))

# Export to PDF with ggplot (It does not work orca) ----

df_plot <- figure_irf_range_correction_pricelevel$data
...
p_out <- ggplot(df_plot, aes(x = period.month)) +
  ...
  labs(title = "Price level response (%)", x = "Month", y = NULL) +
  theme_minimal(base_size = 13) +
  theme(
    legend.position = "none",
    plot.title = element_text(hjust = 0.5, size = 14, color = "grey30"),
...
out_pdf <- here::here(
  paste0("analysis/working_paper_1/figures/irf_range_correction/", subfolder,
         "/figure_irf_range_correction_pricelevel_robust_15.pdf")
)
dir.create(dirname(out_pdf), recursive = TRUE, showWarnings = FALSE)
...
figure_irf_range_correction_rate$data <- figure_irf_range_correction_rate$data %>%
  ...
  left_join(mean_median_per_period, by = "period.month")
# Save figure data as csv
write_csv(figure_irf_range_correction_rate$data, here::here(paste0("analysis/working_paper_1/figures/irf_range_correction/", subfolder, "/figure_irf_range_correction_rate_robust_15.csv")))

# Export to PDF with ggplot (It does not work orca) ----

df_plot <- figure_irf_range_correction_rate$data
...
p_out <- ggplot(df_plot, aes(x = period.month)) +
  ...
  labs(title = "Interest rate response (%-points)", x = "Month", y = NULL) +
  theme_minimal(base_size = 13) +
  theme(
    legend.position = "none",
    plot.title = element_text(hjust = 0.5, size = 14, color = "grey30"),
...
out_pdf <- here::here(
  paste0("analysis/working_paper_1/figures/irf_range_correction/", subfolder,
         "/figure_irf_range_correction_rate_robust_15.pdf")
)
dir.create(dirname(out_pdf), recursive = TRUE, showWarnings = FALSE)
