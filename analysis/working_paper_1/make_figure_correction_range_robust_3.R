...
wins_para_levels_ak <- c(0)  # Restricted range for AK method
...
analyze_fullsample <- function(subsample_id) {
  ...
  # Now we need to handle the missing AK results for wins > 0
  # We'll use the AK results from wins = 0 for higher winsorization levels
  ak_results_max <- results_list[["wins_0.00"]][["AK"]]
  
  for (wins in wins_para_levels[wins_para_levels > 0]) {
