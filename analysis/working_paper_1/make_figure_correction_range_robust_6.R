...
# Robustness check 6: Use only conf at 68% or 1_SE observations ----
d_no_qc <- d_no_qc %>% 
  filter(conf %in% c("68", "1_SE"))

# Sub-folder for figures ----
subfolder <- "conf_68" # Robustness check 6
# This version:
# Methods:
## FAT-PET unweighted
## FAT-PET
...
write_csv(figure_irf_range_correction_output$data, here::here(paste0("analysis/working_paper_1/figures/irf_range_correction/", subfolder, "/figure_irf_range_correction_output_robust_6.csv")))
...
p_out <- ggplot(df_plot, aes(x = period.month)) +
  ...
  coord_cartesian(ylim = c(-2.75, 1)) +
  scale_x_continuous(limits = c(0, 60), breaks = seq(0, 60, 20), expand = c(0, 0)) +
  scale_y_continuous(breaks = seq(-2.5, 1, by = 0.5), expand = c(0, 0)) +
...
out_pdf <- here::here(
  paste0("analysis/working_paper_1/figures/irf_range_correction/", subfolder,
         "/figure_irf_range_correction_output_robust_6.pdf")
)
...
perform_meta_analysis <- function(data, wins, se_opt = "lower", waap_horizon = 36) {
  list(
    fatpet_uw = meta_analysis(
      data = data,
      outvar = out_var,
...
figure_irf_range_correction_pricelevel$data <- figure_irf_range_correction_pricelevel$data %>%
  ...
  left_join(mean_median_per_period, by = "period.month")
# Save figure data as csv
write_csv(figure_irf_range_correction_pricelevel$data, here::here(paste0("analysis/working_paper_1/figures/irf_range_correction/", subfolder, "/figure_irf_range_correction_pricelevel_robust_6.csv")))
...
out_pdf <- here::here(
  paste0("analysis/working_paper_1/figures/irf_range_correction/", subfolder,
         "/figure_irf_range_correction_pricelevel_robust_6.pdf")
)
dir.create(dirname(out_pdf), recursive = TRUE, showWarnings = FALSE)
...
figure_irf_range_correction_rate$data <- figure_irf_range_correction_rate$data %>%
  ...
  left_join(mean_median_per_period, by = "period.month")
# Save figure data as csv
write_csv(figure_irf_range_correction_rate$data, here::here(paste0("analysis/working_paper_1/figures/irf_range_correction/", subfolder, "/figure_irf_range_correction_rate_robust_6.csv")))
...
out_pdf <- here::here(
  paste0("analysis/working_paper_1/figures/irf_range_correction/", subfolder,
         "/figure_irf_range_correction_rate_robust_6.pdf")
)
dir.create(dirname(out_pdf), recursive = TRUE, showWarnings = FALSE)
