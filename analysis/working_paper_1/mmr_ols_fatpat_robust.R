...
mmr_output_ols_fatpet <- meta_analysis(
  d_no_qc,
  outvar = out_var,
  se_option = "upper",
...
modelsummary::modelsummary(
  mmr_output_ols_fatpet[as.character(chosen_periods_tables)],
  output = "gt",
  stars = TRUE,
...
modelsummary::modelsummary(
  mmr_output_ols_fatpet[as.character(chosen_periods_tables)],
  output = paste0(
    "analysis/working_paper_1/tables/mmr_fatpet/",
    "mmr_output_ols_fatpet",
    ".png"
  ),
  stars = FALSE,
  fmt = 3,
...
#### Plots ----
##### Coefficient plots ----
(p1 <- create_mmr_coefficient_plot(
  mmr_output_ols_fatpet,
  "(Intercept)",
...
(p2 <- create_mmr_coefficient_plot(
  mmr_output_ols_fatpet,
  "standarderror_winsor",
...
ggsave(
  here::here(
    "analysis/working_paper_1/figures/mmr/figure_mmr_output_ols_fatpet_intercept_p_bias.pdf"
  ),
...
y_lims <- c(-1.5, 0.75)
p3 <- create_mmr_coefficient_plot(
  mmr_output_ols_fatpet,
  "group_ident_broadhf",
  custom_title = "HF"
) +
  ...
  # No axis labels
  theme(axis.title.x = element_blank(), axis.title.y = element_blank())
p4 <- create_mmr_coefficient_plot(
  mmr_output_ols_fatpet,
  "group_ident_broadnr",
  custom_title = "NR"
) +
  ...
  # No axis labels
  theme(axis.title.x = element_blank(), axis.title.y = element_blank())
p5 <- create_mmr_coefficient_plot(
  mmr_output_ols_fatpet,
  "group_ident_broadsignr",
  custom_title = "SignR"
) +
  coord_cartesian(ylim = y_lims) +
  # No subtitle
  ...
  # No axis labels
  theme(axis.title.x = element_blank(), axis.title.y = element_blank())
p6 <- create_mmr_coefficient_plot(
  mmr_output_ols_fatpet,
  "group_ident_broadidother",
  custom_title = "Other"
) +
  coord_cartesian(ylim = y_lims) +
  # No subtitle
  ...
  theme(axis.title.x = element_blank(), axis.title.y = element_blank())
# Plot on top journal
p7 <- create_mmr_coefficient_plot(
  mmr_output_ols_fatpet,
  "top_5_or_tier",
...
p8 <- create_mmr_coefficient_plot(
  mmr_output_ols_fatpet,
  "cbanker",
...
ggsave(
  here::here(
    "analysis/working_paper_1/figures/mmr/figure_mmr_output_ols_fatpet_moderators.pdf"
  ),
  plot = combined_plot,
  device = "pdf",
  width = 7,
...
mmr_output_baseline_corrected_effects_ident_OLS <- ggplot(
  prediction,
  aes(x = period, color = source, fill = source)
) +
  # # Add confidence intervals without outer lines
  geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper), alpha = 0.1, color = NA) +
  geom_line(aes(y = predicted_value), linewidth = 1) +
  geom_point(aes(y = predicted_value), size = 2) +
  scale_color_manual(
    values = c(
      "Chol/SVAR" = "#112EB8",
...
ggsave(
  here::here(
    "analysis/working_paper_1/figures/mmr/figure_mmr_output_baseline_corrected_effects_ident_OLS.pdf"
  ),
...
mmr_output_baseline_corrected_effects_ident_OLS_with_subsamples <- ggplot(
  prediction,
  aes(x = period, color = source, fill = source)
) +
  # # Add confidence intervals without outer lines
...
ggsave(
  here::here(
    "analysis/working_paper_1/figures/mmr/figure_mmr_output_baseline_corrected_effects_ident_OLS_with_subsamples.pdf"
  ),
...
mmr_output_robust_ols_fatpet <- meta_analysis(
  d_no_qc,
  outvar = out_var,
  se_option = "upper",
  periods = chosen_periods,
...
### OLS-FAT-PET model estimation ----
mmr_pricelevel_ols_fatpet <- meta_analysis(
  d_no_qc,
  outvar = out_var,
  se_option = "upper",
  periods = chosen_periods,
...
##### Create html output
modelsummary::modelsummary(
  mmr_pricelevel_ols_fatpet[as.character(chosen_periods_tables)],
  output = "gt",
  stars = TRUE,
  conf_level = conflevel,
  title = paste("OLS-FAT-PET", out_var),
...
ggsave(
  here::here(
    "analysis/working_paper_1/figures/mmr/figure_mmr_pricelevel_ols_fatpet_intercept_p_bias.pdf"
  ),
...
ggsave(
  here::here(
    "analysis/working_paper_1/figures/mmr/figure_mmr_pricelevel_ols_fatpet_moderators.pdf"
  ),
  plot = combined_plot,
  device = "pdf",
  width = 7,
...
#### Plot corrected effects ----
mmr_pricelevel_baseline_corrected_effects_ident_OLS <- ggplot(
  prediction,
  aes(x = period, color = source, fill = source)
) +
  # Add confidence intervals without outer lines
...
ggsave(
  here::here(
    "analysis/working_paper_1/figures/mmr/figure_mmr_pricelevel_baseline_corrected_effects_ident_OLS.pdf"
  ),
...
# Plot with uncorrected sub-sample lines without bounds
mmr_pricelevel_baseline_corrected_effects_ident_OLS_with_subsamples <- ggplot(
  prediction,
  aes(x = period, color = source, fill = source)
) +
  geom_line(aes(y = predicted_value), linewidth = 1) +
...
ggsave(
  here::here(
    "analysis/working_paper_1/figures/mmr/figure_mmr_pricelevel_baseline_corrected_effects_ident_OLS_with_subsamples.pdf"
  ),
...
### Robustness model estimation (also used for BMA comparison) ----
mmr_pricelevel_robust_ols_fatpet <- meta_analysis(
  d_no_qc,
  outvar = out_var,
  se_option = "upper",
  periods = chosen_periods,
